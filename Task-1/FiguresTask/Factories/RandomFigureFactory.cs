...
namespace FiguresTask.Factories
{
    public class RandomFigureFactory : FigureFactoryBase
    {
        ...
        public RandomFigureFactory(double? minArgumentThreshold = null, double? maxArgumentThreshold = null, int? count = null) : base()
        {
        ...
        protected IFigure CreateFigure(string figureType)
        {
            switch (figureType)
            {
                ...
                case "triangle":
                    ...
                    double lowerBound = Math.Max(Math.Max(triangleArgs[0] - triangleArgs[1], triangleArgs[1] - triangleArgs[0]), this.minThreshold);
